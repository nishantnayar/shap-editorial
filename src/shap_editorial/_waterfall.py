...
from ._utils import extract_single_explanation
...
def waterfall(
    ...
    *,
    ...
    max_display : int
        Number of top features (by |SHAP|) to show individually.
    show_other : bool
    ...
    # contributions), then features in ascending impact so the largest sits at
    # the top and its bar lands on f(x).
    rows = []
    if has_other:
    ...
    # Cumulative geometry: each bar starts where the previous one ended.
    running = base
    starts, ends = [], []
    ...
    # Reference lines: E[f(x)] (start) and f(x) (final output).
    ...
    pad = 0.18 * (span_hi - span_lo or 1.0)
    ax.set_xlim(span_lo - pad, span_hi + pad)

    # Plain-language endpoint labels above their reference lines (at the top, so
    # they never collide with the x-axis ticks). No "E[f(x)]"/"f(x)" jargon.
    ax.annotate(
